-- ============================================================
-- AI Study Partner - RPC Functions
-- ============================================================
...
begin
  ...
  update garden_stats set
    ...
    plants_grown_total = plants_grown_total + (case when v_stage = 4 and garden_stats.current_plant_stage < 4 then 1 else 0 end),
    garden_layout = case
      when v_stage = 4 and garden_stats.current_plant_stage < 4
        then garden_layout || jsonb_build_object('grown_at', now(), 'streak', v_streak)
      else garden_layout
    end
  where garden_stats.user_id = p_user_id;
